...
public final class Helper 
{
	 ...
	 * Si no está establecido, lo inicializa leyendo su valor del archivo de propiedades utilities.properties.
	 * 
	 * @return
	 */
	public static String getDefaultDatePattern()
 	{
 		if(Helper.defaultDatePattern == null)
 		{
			...
			try 
			{
				pfr = PropertiesFileReaderFactory.getPropertiesFileReader("utilities.properties");
				Helper.defaultDatePattern = pfr.getProperty("DefaultDatePattern");
			} 
			catch (IOException e) 
			{
				logger.debug("El método getDefaultDatePattern() intentó obtener el patrón por defecto para el formateo de fechas del archivo de propiedades utilities.properties, pero este no existe. Se establece el patrón por defecto a dd/MM/yyyy.");
			}
 		}
